...
public class PerfectlySphericalHousesInAVacuum {
	...
	private static void firstPart(String steps) {
		...
		Coord at = new Coord(0, 0);
		visited.add(at);
		for (int i = 0; i < steps.length(); ++i) {
			at = at.move(steps.substring(i, i + 1));
			visited.add(at);
		}
	...
	private static void secondPart(String steps) {
		Set<Coord> visited = new HashSet<>();
		Coord[] ats = new Coord[] { new Coord(0, 0), new Coord(0, 0) };
		visited.add(ats[0]);
		visited.add(ats[1]);
		for (int i = 0; i < steps.length(); ++i) {
			Coord at = ats[i % 2].move(steps.substring(i, i + 1));
			ats[i % 2] = at;
			visited.add(at);
		}
		System.out.println(visited.size());
	...
	private static record Coord(int x, int y) {
		public Coord move(String dir) {
			return switch (dir) {
			case "^" -> new Coord(x, y + 1);
			case "v" -> new Coord(x, y - 1);
			case ">" -> new Coord(x + 1, y);
			case "<" -> new Coord(x - 1, y);
	...
	private static String readInput() {
		try (var reader = new BufferedReader(
				new InputStreamReader(PerfectlySphericalHousesInAVacuum.class.getResourceAsStream("/day03.txt")))) {
