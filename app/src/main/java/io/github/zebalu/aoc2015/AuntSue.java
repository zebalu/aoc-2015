...
public class AuntSue {
    ...
    public static void main(String[] args) {
        var sues = readSues();
        var mfcsamSue = readMfcsamSue();
        firstPart(sues, mfcsamSue);
        secondPart(sues, mfcsamSue);
    }

    private static void firstPart(Sues sues, Sue mfcsamSue) {
        var foundSueId = sues.entrySet().stream().filter(e -> e.getValue().allows(mfcsamSue)).findAny().orElseThrow()
                .getKey();
        System.out.println(foundSueId);
    }
    
    private static void secondPart(Sues sues, Sue mfcsamSue) {
        var foundRangedSueId = sues.entrySet().stream().filter(e -> e.getValue().allowsRange(mfcsamSue)).findAny()
                .orElseThrow().getKey();
        System.out.println(foundRangedSueId);
    }

    private static Sues readSues() {
        var sues = new Sues();
        readInput().forEach(line -> {
            var m = SUE_PATTERN.matcher(line);
            if (!m.matches()) {
                throw new IllegalArgumentException("can not parse line: '" + line + "'");
            }
            Sue sue = sues.computeIfAbsent(Integer.parseInt(m.group(1)), k -> new Sue());
            var attributes = Arrays.asList(m.group(2).split(", "));
            parseSueAttributes(sue, attributes);
        });
        return sues;
    }

    private static void parseSueAttributes(Sue sue, List<String> attributes) {
        for (String att : attributes) {
            var kv = att.split(": ");
            sue.put(kv[0], Integer.parseInt(kv[1]));
        }
    }

    private static final class Sue extends LinkedHashMap<String, Integer> {
        private static final long serialVersionUID = 1L;
        private static final Set<String> GREATER_THAN_KEYS = Set.of("cats", "trees");
        private static final Set<String> LESS_THAN_KEYS = Set.of("pomeranians", "goldfish");

        boolean allows(String key, int value) {
            if (containsKey(key)) {
        ...
        boolean allows(Sue otherSue) {
            return otherSue.entrySet().stream().map(e -> allows(e.getKey(), e.getValue())).reduce(true,
                    (a, v) -> a && v);
        }

        boolean allowsRange(String key, int value) {
            if (containsKey(key)) {
                if (GREATER_THAN_KEYS.contains(key)) {
                    return get(key) > value;
                }
                if (LESS_THAN_KEYS.contains(key)) {
                    return get(key) < value;
                }
                return get(key) == value;
            }
        ...
        boolean allowsRange(Sue otherSue) {
            return otherSue.entrySet().stream().map(e -> allowsRange(e.getKey(), e.getValue())).reduce(true,
                    (a, v) -> a && v);
        }
    }

    /** typedf */
    private static final class Sues extends LinkedHashMap<Integer, Sue> {
        private static final long serialVersionUID = 1L;
    }

    private static final List<String> readInput() {
        try (var reader = new BufferedReader(new InputStreamReader(AuntSue.class.getResourceAsStream("/day16.txt")))) {
    ...
    private static final Sue readMfcsamSue() {
        Sue sue = new Sue();
        parseSueAttributes(sue, MFCSAM.lines().toList());
        return sue;
    }

    private static final String MFCSAM = """
            children: 3
            cats: 7
            samoyeds: 2
            pomeranians: 3
