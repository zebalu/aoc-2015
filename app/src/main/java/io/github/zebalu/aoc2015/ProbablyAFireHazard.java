...
public class ProbablyAFireHazard {
	...
	public static void main(String[] args) {
		var changes = readInput();
		firstPart(changes);
		secondPart(changes);
	}

	private static void firstPart(List<Change> changes) {
		Map<Coord, Boolean> lights = new HashMap<>();
		for (int x = 0; x < 1000; ++x) {
			for (int y = 0; y < 1000; ++y) {
				lights.put(new Coord(x, y), Boolean.FALSE);
			}
		}
		changes.forEach(change -> {
			for (int x = change.from().x(); x <= change.to().x(); ++x) {
				for (int y = change.from().y(); y <= change.to().y(); ++y) {
					executeSwitch(lights, change.command(), new Coord(x, y));
				}
			}
		});
		var on = lights.entrySet().stream().filter(Entry::getValue).count();
		System.out.println(on);
	}
	
	private static void secondPart(List<Change> changes) {
		Map<Coord, Integer> lights = new HashMap<>();
		for (int x = 0; x < 1000; ++x) {
			for (int y = 0; y < 1000; ++y) {
				lights.put(new Coord(x, y), 0);
			}
		}
		changes.forEach(change -> {
			for (int x = change.from().x(); x <= change.to().x(); ++x) {
				for (int y = change.from().y(); y <= change.to().y(); ++y) {
					executeIncrease(lights, change.command(), new Coord(x, y));
				}
			}
		});
		var totalBrightness = lights.entrySet().stream().mapToLong(Entry::getValue).sum();
		System.out.println(totalBrightness);
	}

	private static void executeSwitch(Map<Coord, Boolean> lights, Command command, Coord coord) {
		switch (command) {
		case OFF -> lights.put(coord, Boolean.FALSE);
		case ON -> lights.put(coord, Boolean.TRUE);
		case TOOGLE -> lights.put(coord, !lights.get(coord));
		}
	}
	
	private static void executeIncrease(Map<Coord, Integer> lights, Command command, Coord coord) {
		int original = lights.get(coord);
		switch (command) {
		case OFF -> {if (original>0) lights.put(coord, original-1);}
		case ON -> lights.put(coord, original+1);
		case TOOGLE -> lights.put(coord, original+2);
		}
	}
	...
	private static record Change(Command command, Coord from, Coord to) {

		private static Pattern CHANGE_PATTERN = Pattern.compile("(.+) (\\d+),(\\d+) through (\\d+),(\\d+)");
		static Change read(String str) {
			var m = CHANGE_PATTERN.matcher(str);
			m.matches();
			...
			case "turn off" -> new Change(Command.OFF, new Coord(fx, fy), new Coord(tx, ty));
			case "turn on" -> new Change(Command.ON, new Coord(fx, fy), new Coord(tx, ty));
			case "toggle" -> new Change(Command.TOOGLE, new Coord(fx, fy), new Coord(tx, ty));
			default -> throw new IllegalArgumentException(str);
			};
		}
	}

	private static List<Change> readInput() {
		try (var reader = new BufferedReader(
				new InputStreamReader(ProbablyAFireHazard.class.getResourceAsStream("/day06.txt")))) {
			return reader.lines().map(Change::read).collect(Collectors.toList());
		} catch (Exception e) {
			return INPUT.lines().map(Change::read).collect(Collectors.toList());
		}
	}
