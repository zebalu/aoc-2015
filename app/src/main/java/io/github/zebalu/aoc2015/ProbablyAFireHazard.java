...
public class ProbablyAFireHazard {
    ...
    private static void firstPart(List<Change> changes) {
        BitSet lights = new BitSet(1000 * 1000);
        changes.forEach(change -> {
            for (int x = change.from().x(); x <= change.to().x(); ++x) {
                for (int y = change.from().y(); y <= change.to().y(); ++y) {
                    executeSwitch(lights, change.command(), x, y);
                }
            }
        });
        var on = IntStream.range(0, lights.size()).filter(lights::get).count();
    ...
    private static void secondPart(List<Change> changes) {
        ...
        changes.forEach(change -> {
            for (int x = change.from().x(); x <= change.to().x(); ++x) {
                for (int y = change.from().y(); y <= change.to().y(); ++y) {
                    executeIncrease(lights, change.command(), x, y);
                }
            }
        });
        var totalBrightness = LongStream.range(0, lights.length).map(l -> lights[(int) l]).sum();
        System.out.println(totalBrightness);
    }

    private static void executeSwitch(BitSet lights, Command command, int x, int y) {
        int coord = convertToLinearCoord(x, y);
        switch (command) {
        case OFF -> lights.clear(coord);
        case ON -> lights.set(coord);
        case TOOGLE -> lights.flip(coord);
        }
    }

    private static void executeIncrease(int[] lights, Command command, int x, int y) {
        int coord = convertToLinearCoord(x, y);
        int original = lights[coord];
        switch (command) {
        case OFF -> lights[coord] = Math.max(0, original - 1);
        case ON -> lights[coord] = original + 1;
        case TOOGLE -> lights[coord] = original + 2;
        }
    }
    ...
    private static int convertToLinearCoord(int x, int y) {
        return x * 1000 + y;
    }
